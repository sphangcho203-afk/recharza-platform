...
export function OrderTracker({ orderId }: { orderId: string }) {
  ...
  return (
    <div className="grid min-w-0 gap-5 lg:grid-cols-[0.75fr_1.25fr]">
      <form
        onSubmit={submit}
        className="min-w-0 rounded-[2rem] border border-slate-200 bg-white p-4 shadow-xl sm:p-6"
      >
        <p className="text-xs font-bold uppercase tracking-[0.18em] text-violet-600">
          Private order access
        </p>
        <h2 className="mt-2 break-all text-xl font-bold tracking-tight text-slate-900 sm:text-2xl">
          {orderId}
        </h2>
        ...
        <label className="mt-5 block text-sm font-bold text-slate-900">
          ...
          <textarea
            ...
            className="mt-2 w-full resize-none rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 font-mono text-sm font-medium text-slate-900 outline-none transition-all placeholder:text-slate-400 focus:border-violet-600 focus:bg-white focus:ring-4 focus:ring-violet-600/10 shadow-inner"
          />
        </label>
        ...
        <button
          ...
          className="mt-3 min-h-12 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-slate-700 shadow-sm transition-all duration-300 hover:bg-slate-50"
        >
        ...
        <button
          ...
          className="mt-3 min-h-12 w-full rounded-xl bg-violet-600 px-5 py-3.5 text-sm font-bold text-white shadow-md transition-all duration-300 hover:bg-violet-700 hover:-translate-y-0.5 disabled:cursor-wait disabled:opacity-60"
        >
      ...
      <section className="min-w-0 rounded-[2rem] border border-slate-200 bg-white p-4 shadow-xl sm:p-6">
        {order ? (
          <>
            <div className="flex flex-col justify-between gap-4 border-b border-slate-100 pb-5 sm:flex-row sm:items-start">
              <div className="min-w-0">
                ...
                <p className="mt-2 break-words text-sm text-slate-500 font-medium">
              ...
              <span className="w-fit shrink-0 rounded-lg border border-emerald-100 bg-emerald-50 px-3 py-1 text-xs font-bold uppercase tracking-wider text-emerald-600 shadow-sm">
            ...
            <dl className="mt-5 grid gap-3 text-sm sm:grid-cols-2">
              ...
              <div className="rounded-xl border border-slate-100 bg-slate-50/50 p-4 shadow-sm">
                <dt className="text-xs font-bold uppercase tracking-wider text-slate-400">Market</dt>
                <dd className="mt-1 break-words font-bold text-slate-900">
                  {getMarketLabel(order)}
                </dd>
              </div>
              <div className="rounded-xl border border-slate-100 bg-slate-50/50 p-4 shadow-sm">
                <dt className="text-xs font-bold uppercase tracking-wider text-slate-400">Receipt</dt>
                <dd className="mt-1 break-words font-bold text-slate-900">
                  {order.customerEmail}
                </dd>
              </div>
              <div className="rounded-xl border border-slate-100 bg-slate-50/50 p-4 shadow-sm">
                <dt className="text-xs font-bold uppercase tracking-wider text-slate-400">Supplier product</dt>
            ...
            <div className="mt-7">
              <div className="flex items-end justify-between gap-4">
                <h3 className="text-lg font-bold text-slate-900">Fulfilment</h3>
                  ...
                  >
                    <div className="flex flex-col justify-between gap-1 sm:flex-row">
                      ...
                      <span className={`text-xs font-bold uppercase ${
                        attempt.status === "failed" ? "text-rose-600" : 
                        attempt.status === "completed" ? "text-emerald-600" : "text-slate-500"
                      }`}>
                        {attempt.status.replaceAll("_", " ")}
                      </span>
                    </div>
                    <p className={`mt-2 break-words text-sm leading-6 font-medium ${
                      attempt.status === "failed" ? "text-rose-700/80" : "text-slate-600"
                    }`}>
                    ...
                    <time className="mt-2 block text-xs text-slate-400 font-bold">
                      {new Date(attempt.updatedAt).toLocaleString()}
                    </time>
                  </article>
                ))}
                {!order.fulfilment.length ? (
                  <div className="rounded-xl border border-dashed border-slate-200 bg-slate-50/50 p-4 text-sm text-slate-400 font-medium text-center">
            ...
            <div className="mt-7">
              ...
              <div className="mt-4 space-y-3">
                {order.events.map((event) => (
                  <article
                    key={event.id}
                    className="rounded-xl border border-slate-100 bg-slate-50/30 p-4 shadow-sm"
                  >
                    <div className="flex flex-col justify-between gap-1 sm:flex-row">
                      <p className="break-words text-xs font-bold uppercase tracking-wider text-violet-600">
                        {event.type.replaceAll("_", " ")}
                      </p>
                      <time className="text-xs text-slate-400 font-bold">
                        {new Date(event.createdAt).toLocaleString()}
                      </time>
                    </div>
                    <p className="mt-2 break-words text-sm leading-6 text-slate-600 font-medium">
                      {event.message}
                    </p>
                  </article>
                ))}
          ...
          <div className="grid min-h-80 place-items-center rounded-2xl border border-dashed border-slate-200 bg-slate-50/50 p-8 text-center">
            <div>
              <p className="text-4xl font-bold text-slate-200">RZ</p>
