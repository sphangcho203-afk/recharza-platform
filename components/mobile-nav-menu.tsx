...
export function MobileNavMenu() {
  ...
  const menu = open ? (
    <div className="recharza-scrim fixed inset-0 z-[9999] isolate" onMouseDown={(event) => { if (event.target === event.currentTarget) closeMenu(); }}>
      <aside id="mobile-navigation" role="dialog" aria-modal="true" aria-labelledby={supportOpen ? "support-chooser-title" : "mobile-navigation-title"} className="recharza-sheet relative h-full w-[280px] overflow-hidden shadow-[20px_0_60px_rgba(0,0,0,0.6)]">
        <span aria-hidden="true" className="pointer-events-none absolute inset-0 overflow-hidden">
          <span className="recharza-atmo-v2 absolute inset-0 opacity-[0.35]" />
          <span style={{ position: "absolute", left: "-20%", top: "-20%", width: "140%", height: "140%", background: "radial-gradient(circle at 30% 30%, rgba(141,92,255,0.12), transparent 60%), radial-gradient(circle at 70% 70%, rgba(34,211,238,0.08), transparent 60%)", filter: "blur(90px)", animation: "recharza-aurora-drift-a 25s ease-in-out infinite alternate" }} />
        </span>
