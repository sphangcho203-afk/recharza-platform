...
const inputClassName =
  "mt-2 min-h-12 w-full rounded-lg border border-slate-200 bg-slate-50 px-3.5 text-sm text-slate-900 outline-none transition duration-150 placeholder:text-slate-400 focus:bg-white focus:border-violet-600 focus:ring-2 focus:ring-violet-500/10 disabled:cursor-not-allowed disabled:opacity-50";
...
function GoogleContinue({ className = "" }: { className?: string }) {
  return (
    <a
      ...
      className={`flex min-h-12 w-full items-center justify-center gap-3 rounded-lg border border-slate-200 bg-white px-5 text-sm font-bold text-slate-700 shadow-sm transition duration-150 ease-out hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 active:scale-[0.99] ${className}`}
    >
      <span aria-hidden="true" className="grid h-5 w-5 place-items-center rounded-full border border-slate-200 bg-white text-xs font-bold text-blue-600 shadow-sm">G</span>
...
function GoogleOutcomeBanner({ outcome, returnTo }: { outcome: GoogleAuthOutcome; returnTo: string }) {
  ...
  return (
    <div role="status" className="rounded-xl border border-emerald-100 bg-emerald-50 px-4 py-3.5 shadow-sm">
      ...
      <p className="mt-2 text-sm leading-6 text-emerald-900/80">{content.message}</p>
    </div>
  );
}
...
export function CustomerAccountShell({ showOrders = false, returnTo = "/account", googleAuth = null }: { showOrders?: boolean; returnTo?: string; googleAuth?: GoogleAuthOutcome | null }) {
  ...
  if (signupSuccess) {
    return (
      <section className="mx-auto max-w-2xl rounded-2xl border border-slate-200 bg-white p-6 shadow-xl sm:p-8" aria-labelledby="account-created-heading">
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="mt-2 text-sm leading-6 text-slate-500 font-medium">Your account is ready for checkout, order tracking, receipts, and support.</p>
          </div>
        </div>
        <dl className="mt-6 grid gap-px overflow-hidden rounded-xl border border-slate-200 bg-slate-50 sm:grid-cols-2 shadow-inner">
        ...
        <button
          ...
          className="mt-6 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-violet-600 px-5 text-sm font-bold text-white shadow-md transition duration-150 ease-out hover:bg-violet-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 active:scale-[0.99]"
        >
          Open my account
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-lg">
      <div className="grid lg:grid-cols-[0.78fr_1.22fr]">
        <aside className="relative hidden overflow-hidden border-r border-slate-100 bg-slate-50/50 p-8 lg:block">
          <div className="relative">
            ...
            <p className="mt-3 text-sm leading-6 text-slate-500">{showOrders ? "Only the signed-in customer can access order history, player destinations, and delivery status." : "Keep checkout, orders, receipts, and support together wherever you play."}</p>
            ...
            <p className="mt-10 text-xs leading-5 text-slate-400">Never share your password, OTP, UPI PIN, or card PIN with support.</p>
          </div>
        </aside>
        ...
        <div className="p-5 sm:p-8 lg:p-10">
          ...
          <div className="mt-6 grid grid-cols-2 gap-1 rounded-xl border border-slate-200 bg-slate-50 p-1 lg:mt-0 shadow-sm" role="tablist" aria-label="Account access mode">
            <button
              type="button"
              role="tab"
          ...
          <div className="mt-7">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-500">{showOrders ? "Sign in to see your account-owned orders. Guest orders remain available through secure order lookup." : mode === "login" ? "Sign in to checkout, track orders, and keep support connected." : "One account for checkout, tracking, receipts, and support."}</p>
          </div>
          ...
          {message ? (
            <p role={error ? "alert" : "status"} aria-live="polite" className={`mt-5 rounded-xl border px-3.5 py-3 text-sm font-bold leading-5 shadow-sm ${error ? "border-rose-100 bg-rose-50 text-rose-600" : "border-emerald-100 bg-emerald-50 text-emerald-700"}`}>
              {message}
            </p>
          ) : null}

          <p className="mt-6 text-center text-xs leading-5 text-slate-500 font-medium">By continuing, you agree to Recharza’s <Link href="/policies/terms" className="text-violet-600 underline underline-offset-4 hover:text-violet-700">Terms</Link> and <Link href="/policies/privacy" className="text-violet-600 underline underline-offset-4 hover:text-violet-700">Privacy Policy</Link>.</p>
        </div>
      </div>
    </section>
  );
...
function PasswordInput({ id, value, onChange, visible, onToggle, autoComplete, placeholder = "Your password" }: { id: string; value: string; onChange: (value: string) => void; visible: boolean; onToggle: () => void; autoComplete: string; placeholder?: string }) {
  return (
    <span className="relative block">
      ...
      <button type="button" onClick={onToggle} aria-label={visible ? "Hide password" : "Show password"} className="absolute right-2 top-1/2 min-h-9 -translate-y-1/2 rounded-lg px-2.5 text-xs font-bold text-slate-400 transition hover:bg-slate-100 hover:text-slate-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500">
        {visible ? "Hide" : "Show"}
      </button>
    </span>
  );
...
function Benefit({ icon, title, text }: { icon: Parameters<typeof StorefrontIcon>[0]["name"]; title: string; text: string }) {
  return (
    <div className="flex gap-3">
      ...
      <div><p className="font-bold text-slate-900">{title}</p><p className="mt-0.5 text-xs leading-5 text-slate-500 font-medium">{text}</p></div>
    </div>
  );
}

function Info({ label, value }: { label: string; value: string }) {
  return <div className="bg-slate-50 p-3.5 border-r border-slate-200 last:border-r-0"><dt className="text-[10px] font-bold uppercase tracking-[0.12em] text-slate-400">{label}</dt><dd className="mt-1 break-all text-sm font-bold text-slate-900">{value}</dd></div>;
}
