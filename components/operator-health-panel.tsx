  ...
  return (
    <section className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-5 sm:p-7">
      <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-start">
        ...
        <div className="w-full max-w-md rounded-lg border border-white/10 bg-black/15 p-4">
          ...
          <label className="mt-4 block text-sm font-semibold text-slate-200">
            ...
            <input
              ...
              className="mt-2 w-full rounded-lg border border-white/10 bg-black/20 px-4 py-3 font-mono text-sm font-normal text-white outline-none placeholder:text-slate-600 focus:border-violet-400"
            />
          </label>
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            <button type="button" onClick={useSavedToken} className="rounded-lg border border-white/10 bg-white/5 px-4 py-3 text-sm font-bold text-slate-200 hover:bg-white/10">
              Use saved fallback
            </button>
            <button type="button" disabled={loading} onClick={() => void loadHealth()} className="rounded-lg bg-white px-4 py-3 text-sm font-semibold text-slate-950 disabled:cursor-wait disabled:opacity-60">
      ...
      <p aria-live="polite" className={`mt-5 rounded-lg border px-4 py-3 text-sm ${isError ? "border-rose-400/20 bg-rose-400/10 text-rose-200" : "border-white/10 bg-black/15 text-slate-400"}`}>
        {message}
      </p>
      ...
      {health?.database && health.accounts && health.payment && health.supplier ? (
        <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <article className="rounded-lg border border-white/10 bg-black/15 p-5">
            ...
            <h3 className="mt-4 text-lg font-semibold text-white">Accounts</h3>
          ...
          <article className="rounded-lg border border-white/10 bg-black/15 p-5">
            ...
            <h3 className="mt-4 text-lg font-semibold text-white">Payments</h3>
            <dl className="mt-4 grid gap-2 text-sm">
          ...
          <article className="rounded-lg border border-white/10 bg-black/15 p-5">
            ...
            <h3 className="mt-4 text-lg font-semibold text-white">Supplier catalogue</h3>
            <dl className="mt-4 grid gap-2 text-sm">
          ...
          <article className="rounded-lg border border-white/10 bg-black/15 p-5">
            ...
            <h3 className="mt-4 text-lg font-semibold text-white">Fulfilment</h3>
            <dl className="mt-4 grid gap-2 text-sm">
