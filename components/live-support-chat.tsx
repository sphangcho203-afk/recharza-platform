...
export function LiveSupportChat({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <div className={embedded ? "w-full" : "fixed bottom-5 right-4 z-[70] sm:bottom-6 sm:right-6"}>
          ...
          <header className="relative overflow-hidden border-b border-white/[0.09] px-5 py-4">
            ...
            <div className="relative flex items-start justify-between gap-3">
              <div className="flex items-center gap-3">
                <span className="relative grid h-11 w-11 place-items-center rounded-lg border border-violet-300/25 bg-violet-400/10 text-lg text-violet-100 shadow-[0_0_28px_rgba(139,92,246,0.2)]" aria-hidden="true">
                  ✦
                  <span className="absolute bottom-0 right-0 h-2.5 w-2.5 rounded-full border-2 border-[#0b0d15] bg-emerald-300" />
                </span>
                <div>
                  <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-violet-200">Live support</p>
                  <h2 className="mt-0.5 text-base font-semibold tracking-[-0.02em] text-white">Recharza concierge</h2>
          ...
          <div ref={scrollRef} className="min-h-0 flex-1 space-y-3 overflow-y-auto px-4 py-4" aria-live="polite">
            {messages.map((message) => (
              <div key={message.id} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[88%] rounded-lg px-3.5 py-3 text-[13px] leading-5 ${message.role === "user" ? "rounded-br-md bg-violet-500 text-white" : "rounded-bl-md border border-white/[0.08] bg-white/[0.045] text-slate-200"}`}>
                  {message.text}
                </div>
              </div>
            ))}
            {messages.length === 1 ? (
              <div className="space-y-2 pt-1">
                <p className="px-1 text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-600">Try asking</p>
                {quickPrompts.map((prompt) => (
                  <button key={prompt} type="button" onClick={() => sendMessage(prompt)} className="block w-full rounded-lg border border-white/[0.08] bg-white/[0.025] px-3 py-2.5 text-left text-xs font-bold text-slate-400 transition hover:border-violet-300/30 hover:bg-violet-400/[0.07] hover:text-white">{prompt}</button>
                ))}
              </div>
            ) : null}
            {sending ? <div className="flex justify-start"><div className="rounded-lg rounded-bl-md border border-white/[0.08] bg-white/[0.045] px-3.5 py-3 text-xs text-slate-500">Thinking through that…</div></div> : null}
          </div>
      ...
      {!embedded ? <button type="button" onClick={() => setOpen((current) => !current)} className={`group ml-auto flex min-h-14 items-center gap-3 rounded-lg border px-4 shadow-[0_16px_40px_rgba(0,0,0,0.3)] transition hover:-translate-y-0.5 ${open ? "border-white/[0.15] bg-[#11131d]" : "border-violet-300/25 bg-violet-500 text-white hover:bg-violet-400"}`} aria-expanded={open} aria-controls="live-support-message">
        <span className={`grid h-9 w-9 place-items-center rounded-lg ${open ? "bg-white/[0.08] text-violet-200" : "bg-white/15 text-white"}`} aria-hidden="true">✦</span>
        <span className="pr-1 text-left"><span className={`block text-[10px] font-semibold uppercase tracking-[0.14em] ${open ? "text-violet-200" : "text-violet-100"}`}>Need a hand?</span><span className="block text-sm font-semibold">Live support</span></span>
      </button> : null}
    </div>
  );
}
