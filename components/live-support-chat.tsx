...
export function LiveSupportChat({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <div className={embedded ? "w-full" : "fixed bottom-5 right-4 z-[70] sm:bottom-6 sm:right-6"}>
      {open ? (
        ...
        >
          <header className="relative overflow-hidden border-b border-slate-100 px-5 py-4 bg-slate-50/50">
            <div className="relative flex items-start justify-between gap-3">
              <div className="flex items-center gap-3">
                <span className="relative grid h-11 w-11 place-items-center rounded-lg border border-violet-200 bg-violet-50 text-lg text-violet-600 shadow-sm" aria-hidden="true">
                  ✦
          ...
          <div ref={scrollRef} className="min-h-0 flex-1 space-y-3 overflow-y-auto px-4 py-4" aria-live="polite">
            {messages.map((message) => (
              <div key={message.id} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[88%] rounded-xl px-3.5 py-3 text-[13px] leading-5 shadow-sm ${message.role === "user" ? "rounded-br-sm bg-violet-600 text-white" : "rounded-bl-sm border border-slate-200 bg-slate-50 text-slate-700 font-medium"}`}>
                  {message.text}
                </div>
              </div>
            ))}
          ...
          <div className="border-t border-slate-100 p-3 bg-white">
            <form onSubmit={(event) => { event.preventDefault(); void sendMessage(); }} className="flex items-end gap-2 rounded-xl border border-slate-200 bg-slate-50 p-1.5 focus-within:border-violet-500 focus-within:ring-2 focus-within:ring-violet-500/10">
              ...
              <button type="submit" disabled={!draft.trim() || sending} className="grid h-10 w-10 shrink-0 place-items-center rounded-lg bg-violet-600 text-white shadow-sm transition hover:bg-violet-700 disabled:cursor-not-allowed disabled:opacity-40" aria-label="Send message"><StorefrontIcon name="arrow" className="h-4 w-4 -rotate-90" /></button>
            </form>
      ...
      {!embedded ? <button type="button" onClick={() => setOpen((current) => !current)} className={`group ml-auto flex min-h-14 items-center gap-3 rounded-xl border px-4 shadow-xl transition-all duration-300 hover:-translate-y-1 ${open ? "border-slate-200 bg-white text-slate-900" : "border-violet-500 bg-violet-600 text-white hover:bg-violet-700"}`} aria-expanded={open} aria-controls="live-support-message">
        <span className={`grid h-9 w-9 place-items-center rounded-lg shadow-sm ${open ? "bg-slate-50 text-violet-600 border border-slate-100" : "bg-white/15 text-white"}`} aria-hidden="true">✦</span>
