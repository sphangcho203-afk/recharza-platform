...
export function MobileLegendsOrderForm({
  ...
  return (
    <form onSubmit={submitOrder} className="grid gap-5">
      <section className="rounded-lg border border-white/10 bg-[radial-gradient(circle_at_top_left,rgba(91,124,255,0.14),transparent_42%),rgba(255,255,255,0.04)] p-4 shadow-2xl shadow-black/20 sm:p-6">
        <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-violet-300">01 · Package</p>
            <h2 className="mt-2 text-2xl font-semibold tracking-tight">Choose a regional offer</h2>
      ...
      <div className="grid gap-5 xl:grid-cols-[0.82fr_1.18fr]">
        <section className="rounded-lg border border-white/10 bg-white/[0.04] p-4 shadow-2xl shadow-black/20 sm:p-6">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-violet-300">02 · Player destination</p>
          <h2 className="mt-2 text-2xl font-semibold tracking-tight">Verify the game account</h2>

          <div className={`mt-4 rounded-lg border px-4 py-3 text-sm ${account.status === "authenticated" ? "border-emerald-400/20 bg-emerald-400/10 text-emerald-100" : "border-amber-300/20 bg-amber-300/10 text-amber-100"}`}>
          ...
          <button
            ...
            className="mt-4 min-h-12 w-full rounded-lg border border-violet-400/30 bg-violet-400/10 px-4 py-3 text-sm font-bold text-violet-100 disabled:cursor-not-allowed disabled:opacity-50"
          >
            {verification.status === "loading" ? "Validating..." : `Validate ${market.label} player`}
          </button>

          <div aria-live="polite" className={`mt-3 rounded-lg border px-4 py-3 text-sm leading-6 ${verification.status === "success" ? "border-emerald-400/20 bg-emerald-400/10 text-emerald-200" : verification.status === "error" ? "border-rose-400/20 bg-rose-400/10 text-rose-200" : "border-white/10 bg-black/15 text-slate-400"}`}>
      ...
      <section className="overflow-hidden rounded-lg border border-white/10 bg-[linear-gradient(145deg,rgba(17,17,29,0.98),rgba(8,8,16,0.98))] shadow-2xl shadow-black/30">
        <div className="grid gap-5 p-4 sm:p-6 lg:grid-cols-[1fr_auto] lg:items-center">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-violet-300">04 · Review</p>
            <h2 className="mt-2 text-2xl font-semibold">Confirm the protected order snapshot</h2>
          ...
          <div className="min-w-56 rounded-lg border border-white/10 bg-black/25 p-4 text-right">
            <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-500">Displayed total</p>
            <p className="mt-1 text-3xl font-semibold text-white">{formatPresentment(selectedPackage.amountInPaise)}</p>
        ...
        <div className="border-t border-white/10 p-4 sm:p-6">
          ...
          {order ? (
            <div aria-live="polite" className="success-rise mt-5 rounded-lg border border-emerald-400/20 bg-emerald-400/10 p-4">
              ...
              <p className="mt-2 break-all text-xl font-semibold text-white">{order.id}</p>
              ...
              <label className="mt-4 block text-xs font-bold uppercase tracking-wider text-emerald-200">
                ...
                <textarea readOnly rows={3} value={order.tracking.accessToken} className="mt-2 w-full resize-none rounded-lg border border-emerald-400/15 bg-black/20 px-3 py-3 font-mono text-xs font-normal normal-case tracking-normal text-emerald-100 outline-none" />
              </label>
              <p className="mt-2 text-xs leading-5 text-emerald-100/70">Keep this token private. It unlocks sensitive tracking details.</p>
              <Link href={order.tracking.path} className="mt-4 block min-h-12 rounded-lg border border-emerald-300/25 bg-emerald-300/10 px-4 py-3 text-center text-sm font-semibold text-emerald-100">Open secure order tracking</Link>
