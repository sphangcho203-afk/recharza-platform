...
export function ResetPasswordForm({ token }: { token: string }) {
  ...
  return (
    <section className="mx-auto max-w-xl overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl shadow-slate-200/50" aria-labelledby="reset-password-heading">
      ...
      <div className="p-8 sm:p-10">
        {!success ? (
          <form onSubmit={submit} className="grid gap-6">
            ...
            <button disabled={submitting || !token} className="min-h-14 rounded-2xl bg-emerald-500 px-6 text-sm font-bold uppercase tracking-widest text-white shadow-xl shadow-emerald-200 transition-all hover:-translate-y-1 hover:bg-emerald-600 active:translate-y-0 disabled:cursor-wait disabled:opacity-50">{submitting ? "Resetting password…" : "Reset password"}</button>
          </form>
        ) : <Link href="/account" className="block min-h-14 rounded-2xl bg-emerald-500 px-6 py-4 text-center text-sm font-bold uppercase tracking-widest text-white shadow-xl shadow-emerald-200 transition-all hover:-translate-y-1 hover:bg-emerald-600">Sign in with the new password</Link>}
        <p role={success ? "status" : "alert"} aria-live="polite" className={`mt-6 rounded-2xl border px-5 py-4 text-sm font-bold leading-relaxed ${success ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-slate-100 bg-slate-50 text-slate-500"}`}>{message}</p>
      </div>
    </section>
  );
}
    ...
    <label htmlFor={id} className="text-[10px] font-bold uppercase tracking-widest text-slate-400">
      ...
      <span className="relative block">
        <input
          ...
          className="mt-3 min-h-14 w-full rounded-2xl border border-slate-200 bg-slate-50 px-5 pr-20 text-base font-bold text-slate-900 outline-none transition-all focus:border-emerald-500 focus:ring-4 focus:ring-emerald-50 placeholder:text-slate-400 disabled:cursor-not-allowed disabled:opacity-50"
        />
        <button
          type="button"
          onClick={onToggle}
          ...
          className="absolute right-3 top-1/2 min-h-9 -translate-y-1/2 rounded-xl px-3 text-[10px] font-bold uppercase tracking-widest text-slate-500 transition-all hover:bg-slate-200 hover:text-slate-900 disabled:opacity-50"
        >
          {visible ? "Hide" : "Show"}
        </button>
      </span>
