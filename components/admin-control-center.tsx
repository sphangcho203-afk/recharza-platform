...
function metricClasses(metric: AdminMetric) {
  if (metric.tone === "positive") return "border-emerald-500/20 bg-emerald-500/10 text-emerald-400";
  if (metric.tone === "warning") return "border-amber-500/20 bg-amber-500/10 text-amber-400";
  if (metric.tone === "danger") return "border-rose-500/20 bg-rose-500/10 text-rose-400";
  return "border-white/10 bg-white/5 text-white";
}

function stateClasses(state: CommandState, danger = false) {
  if (state === "live") return "border-emerald-500/20 bg-emerald-500/10 text-emerald-400";
  if (state === "beta") return "border-cyan-500/20 bg-cyan-500/10 text-cyan-400";
  if (danger) return "border-rose-500/20 bg-rose-500/10 text-rose-400";
  if (state === "planned") return "border-white/5 bg-white/2 text-slate-500";
  return "border-white/5 bg-white/2 text-slate-500";
}

function alertClasses(severity: "info" | "warning" | "critical") {
  if (severity === "critical") return "border-rose-500/20 bg-rose-500/10 text-rose-400";
  ...
  return (
    <section id="control-center" className="mt-8 scroll-mt-24">
      <div className="overflow-hidden rounded-[2.5rem] border border-white/10 bg-white/5 shadow-2xl backdrop-blur-md">
        <div className="border-b border-white/10 px-5 py-5 sm:px-7 bg-white/5">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-violet-400">Master administration layer</p>
              <h2 className="mt-2 text-2xl font-bold tracking-tight text-white sm:text-3xl">Recharza Control Center</h2>
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className="min-h-11 rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-xs font-bold text-white shadow-sm transition hover:bg-white/10"
              >
                Refresh live data
              </button>
              <a
                href="#database"
                className="inline-flex min-h-11 items-center rounded-xl bg-violet-600 px-4 py-2 text-xs font-bold text-white shadow-[0_0_20px_rgba(124,58,237,0.4)] transition hover:bg-violet-700 hover:-translate-y-0.5"
              >
                Open database
              </a>
            </div>
          </div>
          <p className="mt-4 font-mono text-[10px] font-bold uppercase tracking-[0.12em] text-slate-600">
        ...
        <div className="grid gap-3 p-4 sm:grid-cols-2 sm:p-6 xl:grid-cols-4">
          {snapshot.metrics.map((metric) => (
            <article key={metric.id} className={`rounded-xl border p-4 shadow-sm ${metricClasses(metric)}`}>
              ...
              <p className="mt-2 text-xs font-medium leading-5 text-slate-400">{metric.note}</p>
            </article>
          ))}
        </div>
      </div>
      ...
      <div className="mt-5 grid gap-3 xl:grid-cols-[0.8fr_1.2fr]">
        <section className="system-panel p-5 border border-white/10 bg-white/5 shadow-2xl backdrop-blur-md rounded-[2.5rem]">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.16em] text-amber-400">Attention rail</p>
              <h3 className="mt-1 text-lg font-bold text-white">System alerts</h3>
            </div>
