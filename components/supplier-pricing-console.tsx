...
const policyFields: Array<{
...
}> = [
  {
    key: "usdInrRatePaise",
    label: "USD/INR rate",
    helper: "Paise per US dollar. ₹96.50 is stored as 9650.",
    suffix: "paise/USD",
  },
  {
    key: "fxBufferBps",
    label: "FX reserve",
    helper: "Protects against conversion spread and currency movement.",
    suffix: "bps",
  },
  {
    key: "gatewayFeeBps",
    label: "Gateway reserve",
    helper: "Estimated payment processing cost deducted from retail revenue.",
    suffix: "bps",
  },
  {
    key: "targetMarginBps",
    label: "Base margin target",
    helper: "Small packs automatically receive a higher percentage floor.",
    suffix: "bps",
  },
  {
    key: "minimumMarginInPaise",
    label: "Minimum profit",
    helper: "Absolute expected profit floor after reserved gateway cost.",
    suffix: "paise",
  },
  {
    key: "overheadInPaise",
    label: "Per-order overhead",
    helper: "Contribution toward supplier plan, support, and operating cost.",
    suffix: "paise",
  },
  {
    key: "roundingInPaise",
    label: "Price rounding",
    helper: "Prices always round upward by this increment.",
    suffix: "paise",
  },
];
...
export function SupplierPricingConsole() {
  ...
  const [message, setMessage] = useState(
    "Load the pricing policy before synchronizing the supplier catalogue.",
  );
  ...
  function useSavedToken() {
    ...
    if (!saved) {
      setIsError(true);
      setMessage("No saved operator token exists in this browser session.");
      return;
    }
  ...
  async function loadPricing(currentToken = normalizedToken()) {
    if (!currentToken) {
      setIsError(true);
    ...
    try {
      ...
      sessionStorage.setItem("recharza-operator-token", currentToken);
      setPolicy(result.policy);
      setCatalog(result.catalog);
      setMessage("Pricing policy and supplier status loaded.");
    } catch {
      setIsError(true);
  ...
  async function savePolicy(event: FormEvent<HTMLFormElement>) {
    ...
    const currentToken = normalizedToken();
    ...
    try {
      ...
      await loadPricing(currentToken);
    } catch {
      setIsError(true);
    ...
    <section className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-5 shadow-2xl shadow-black/20 sm:p-7">
      <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-start">
        <div>
          ...
          <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-400">
            The API key remains server-side. This panel controls the retail policy and runs a protected
            catalogue sync. Synced categories are published only when their IDs appear in the server allowlist.
          </p>
        </div>

        <div className="grid min-w-60 gap-2 rounded-2xl border border-white/10 bg-black/20 p-4 text-sm">
          <div className="flex justify-between gap-6">
            <span className="text-slate-500">Products synced</span>
            <span className="font-bold text-white">{catalog?.productCount ?? "—"}</span>
          </div>
          <div className="flex justify-between gap-6">
            <span className="text-slate-500">Published live</span>
            <span className="font-bold text-emerald-300">{catalog?.publishedCount ?? "—"}</span>
          </div>
          <div className="flex justify-between gap-6">
            <span className="text-slate-500">Last sync</span>
            <span className="font-bold capitalize text-white">
              {catalog?.latestSync?.status?.toLowerCase() ?? "—"}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-[1fr_auto_auto]">
        ...
        <button
          ...
          className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm font-bold text-slate-200 transition hover:bg-white/10 disabled:opacity-50"
        >
          Use saved token
        </button>
        <button
          ...
          disabled={Boolean(busy)}
          className="rounded-2xl bg-violet-500 px-5 py-3 text-sm font-black text-white transition hover:bg-violet-400 disabled:opacity-50"
        >
      ...
      <form onSubmit={savePolicy} className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {policyFields.map((field) => (
          <label
            key={field.key}
            className="rounded-2xl border border-white/10 bg-black/15 p-4 text-sm font-semibold text-slate-200"
          >
            <span className="flex items-center justify-between gap-3">
              {field.label}
              <span className="text-[10px] uppercase tracking-wider text-slate-600">{field.suffix}</span>
            </span>
            <input
              required
              type="number"
        ...
        <div className="grid gap-3 rounded-2xl border border-white/10 bg-black/15 p-4">
          <button
            type="submit"
            disabled={Boolean(busy)}
            className="rounded-xl bg-gradient-to-r from-violet-500 to-fuchsia-500 px-4 py-3 text-sm font-black text-white disabled:opacity-50"
          >
            {busy === "save" ? "Repricing..." : "Save and reprice"}
          </button>
          <button
            type="button"
            onClick={() => void syncSupplier()}
            disabled={Boolean(busy)}
            className="rounded-xl border border-emerald-300/20 bg-emerald-300/10 px-4 py-3 text-sm font-black text-emerald-100 disabled:opacity-50"
          >
