...
export function GameCatalogue({
  ...
  return (
    <div className="mt-6 sm:mt-8">
      <div className="relative -mx-1 border-b border-white/10 pb-3 sm:pb-4">
        ...
        {filters.map((item) => (
          <Link
            ...
            className={`inline-flex min-h-9 shrink-0 items-center gap-2 rounded-xl border px-4 py-2 text-sm font-bold uppercase tracking-widest transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 ${
              filter === item.id
                ? "border-violet-600 bg-violet-600 text-white shadow-[0_0_20px_rgba(124,58,237,0.4)]"
                : "border-white/10 bg-white/5 text-slate-400 hover:border-white/20 hover:text-white hover:bg-white/10"
            }`}
          >
        ...
        <span aria-hidden="true" className="pointer-events-none absolute right-0 top-0 h-10 w-10 bg-gradient-to-l from-[#0d0f16] to-transparent sm:hidden" />
      </div>
      ...
      {filteredGames.length ? (
        ...
        <div className="mt-6 grid min-h-64 rounded-2xl border-2 border-dashed border-white/10 bg-white/5 p-8 text-center place-items-center">
          <div className="max-w-xs">
            <div className="mx-auto grid h-12 w-12 place-items-center rounded-full bg-white/10">
              <StorefrontIcon name="search" className="h-6 w-6 text-slate-400" />
            </div>
            <p className="mt-4 text-lg font-bold text-white">No matching game</p>
            <p className="mt-2 text-sm font-medium text-slate-400">Try another title, package or category to find what you're looking for.</p>
          </div>
        </div>
      )}
