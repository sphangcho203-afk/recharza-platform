...
export function GameCatalogue({ games }: { games: Game[] }) {
  ...
  const visibleGames = useMemo(() => {
    ...
    return mainGames.filter((game) => {
      ...
      const searchable = [game.title, game.publisher, game.category, ...game.packages]
        .join(" ")
        .toLowerCase();
  ...
  return (
    <div className="mt-8">
      ...
      {showRegions && mlbb ? (
        <section
          id="mlbb-regions"
          className="mt-6 scroll-mt-20 rounded-2xl border border-white/10 bg-[#0d0d16] p-4 sm:p-5"
        >
