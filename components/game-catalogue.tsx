  ...
  return (
    <div className="mt-6 sm:mt-8">
      <div className="relative -mx-1 border-b border-slate-200 pb-3 sm:pb-4">
        <nav aria-label="Catalogue filters" className="flex gap-3 overflow-x-auto px-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {filters.map((item) => (
            <Link
              key={item.id}
              href={item.id === "all" ? "/#games" : `/?category=${item.id}#games`}
              className={`inline-flex min-h-[2.75rem] shrink-0 items-center gap-2.5 rounded-2xl border px-5 text-[11px] font-black uppercase tracking-[0.12em] transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500 ${
                filter === item.id
