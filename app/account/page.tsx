...
export default async function AccountPage({ searchParams }: AccountPageProps) {
  ...
  return (
    <main className="storefront-page recharza-atmo-v2 recharza-atmo-account min-h-screen overflow-x-clip text-slate-900">
      <SiteHeader />
      <section className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8 lg:py-16">
        <Link href="/" className="inline-flex items-center gap-2 text-sm font-bold text-slate-500 transition hover:text-slate-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-300/70">← Back to store</Link>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_0.72fr] lg:items-end">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-base leading-7 text-slate-600">Sign in to checkout, follow deliveries, view receipts, and keep support connected to the right purchase.</p>
          </div>
        ...
        {rawReason === "sign-in" && protectedWorkspace ? <div role="status" className="mt-6 rounded-xl border border-cyan-100 bg-cyan-50 px-4 py-3 text-sm font-medium text-cyan-700">Sign in with an authorised account to continue to the protected workspace.</div> : null}
        {rawReason === "forbidden" ? <div role="alert" className="mt-6 rounded-xl border border-amber-100 bg-amber-50 px-4 py-3 text-sm font-medium text-amber-700">This account does not have permission to open that workspace.</div> : null}
