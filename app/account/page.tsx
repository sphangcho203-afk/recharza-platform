...
export default async function AccountPage({ searchParams }: AccountPageProps) {
  ...
  return (
    <main className="storefront-page recharza-atmo-v2 recharza-atmo-account min-h-screen overflow-x-clip text-white bg-[#06070d]">
      <SiteHeader />
      <section className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8 lg:py-16">
        <Link href="/" className="inline-flex items-center gap-2 text-sm font-bold text-slate-500 transition hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-500">← Back to store</Link>

        <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_0.72fr] lg:items-end">
          <div>
            <div className="flex items-center gap-3">
              <span className="grid h-11 w-11 place-items-center rounded-lg border border-white/10 bg-white/5 shadow-2xl"><RecharzaMark compact /></span>
              <p className="text-xs font-bold uppercase tracking-[0.18em] text-violet-400">Recharza account</p>
            </div>
            <h1 className="mt-5 text-4xl font-bold tracking-tight text-white sm:text-5xl">Your top-ups, in one place.</h1>
        ...
        {rawReason === "sign-in" && protectedWorkspace ? <div role="status" className="mt-6 rounded-xl border border-cyan-500/20 bg-cyan-500/10 px-4 py-3 text-sm font-medium text-cyan-400 backdrop-blur-md">Sign in with an authorised account to continue to the protected workspace.</div> : null}
        {rawReason === "forbidden" ? <div role="alert" className="mt-6 rounded-xl border border-amber-500/20 bg-amber-500/10 px-4 py-3 text-sm font-medium text-amber-400 backdrop-blur-md">This account does not have permission to open that workspace.</div> : null}
