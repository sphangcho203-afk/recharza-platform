...
export default async function PolicyPage({
  ...
  return (
    <main className="min-h-screen bg-[#06070d] text-white">
      ...
      <section className="border-b border-white/5 bg-white/2 backdrop-blur-md">
        <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
          <Link
            href="/"
            className="inline-flex min-h-10 items-center gap-2 rounded-lg text-sm font-bold text-violet-400 transition hover:text-white"
          >
            ← Back to Recharza
          </Link>
      ...
      <div className="mx-auto grid max-w-5xl gap-8 px-4 py-10 sm:px-6 sm:py-14 lg:grid-cols-[minmax(0,1fr)_17rem] lg:px-8">
        <article className="grid gap-4">
          {paragraphs.map((paragraph) => (
            <p
              ...
              className="whitespace-pre-line rounded-xl border border-white/10 bg-white/5 p-5 text-sm leading-7 text-slate-300 sm:p-6 sm:text-[0.95rem] font-medium shadow-2xl backdrop-blur-md"
            >
              {paragraph}
            </p>
          ))}
        ...
        <aside className="h-fit rounded-[2rem] border border-white/10 bg-white/5 p-5 lg:sticky lg:top-24 shadow-2xl backdrop-blur-md">
          <RecharzaMark />
          <p className="mt-4 text-sm leading-6 text-slate-400 font-medium">
          ...
          <div className="mt-5 grid gap-2">
            <Link
              href="/orders/lookup"
              className="inline-flex min-h-11 items-center justify-between rounded-xl border border-white/10 bg-white/5 px-3.5 text-sm font-bold text-white transition hover:bg-white/10 shadow-sm"
            >
              Track order
          ...
          <p className="mt-5 border-t border-white/10 pt-4 text-[11px] leading-5 text-slate-600 font-medium">
