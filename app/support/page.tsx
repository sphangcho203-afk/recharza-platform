...
export default function SupportPage() {
  ...
  return (
    <main className="storefront-page min-h-screen overflow-x-clip text-slate-900 bg-white">
      ...
      <section id="live-chat" className="border-b border-slate-100 bg-white px-4 py-8 sm:px-6 sm:py-10 lg:px-8">
        <div className="mx-auto grid max-w-[1100px] gap-4 lg:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)] lg:items-start">
          <div className="rounded-2xl border border-slate-200 bg-slate-50/50 p-5 sm:p-6 shadow-sm">
            ...
            <div className="mt-5 flex flex-wrap items-center gap-2">
              <TelegramGroupLauncher className="inline-flex min-h-11 items-center justify-center rounded-xl bg-cyan-600 px-4 text-sm font-bold text-white shadow-md transition-all duration-300 hover:bg-cyan-700 hover:-translate-y-0.5">Open live support in Telegram</TelegramGroupLauncher>
              <a href="https://t.me/supprtrz" target="_blank" rel="noopener noreferrer" className="inline-flex min-h-11 items-center justify-center rounded-xl border border-slate-200 bg-white px-4 text-xs font-bold text-slate-700 shadow-sm transition-all duration-300 hover:bg-slate-50">Use Telegram Web <span className="ml-2" aria-hidden="true">↗</span></a>
            </div>
          </div>
      ...
      <section id="how-support-works" className="border-b border-slate-100 bg-slate-50/30 px-4 py-8 sm:px-6 sm:py-10 lg:px-8">
        <div className="mx-auto max-w-[1100px]">
          <SupportExplainer />
        </div>
      </section>
      ...
      <section className="border-y border-amber-200 bg-amber-50 px-4 py-5 sm:px-6 lg:px-8">
        <div className="mx-auto flex max-w-[1100px] items-start gap-3">
          ...
          <div>
            <h2 className="text-sm font-bold text-amber-700">Support will never request sensitive security codes.</h2>
            <p className="mt-1 text-xs leading-5 text-amber-900/60 font-medium">Never send a password, OTP, UPI PIN, card PIN, sign-in link or remote-device access code.</p>
          </div>
        </div>
      </section>
